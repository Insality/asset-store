...
---@param source_folder string|nil Store source folder to replace (e.g. "widget", "system")
---@return string modified Modified content
local function replace_paths_in_content(content, author, install_folder, source_folder)
	...
	local default_folder_path = source_folder:gsub("^/", "")
...
function M.process_widget_paths(folder_path, install_folder, widget_id, author, file_list, source_folder)
	...
	for _, file_path_in_zip in ipairs(file_list) do
		...
		if not content then
			...
			local modified_content = replace_paths_in_content(content, author, install_folder, source_folder)
